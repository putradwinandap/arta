...
export function HouseholdInvitePanel({ householdId }: { householdId: string }) {
  ...
  async function create() {
    ...
    try {
      ...
      setStatus("Invitation created. It can be used once and expires in 7 days.");
    } catch (error) {
      setStatus(
  ...
  async function copy() {
    try {
      ...
      setStatus("Copy is unavailable; select and copy the code manually.");
    }
  }
  return (
    <section className="panel" aria-label="Household invitations">
      <p className="eyebrow">Family management</p>
      <h2>Invite someone</h2>
      <p className="muted">
        Create a single-use invite code for a family member. The code expires in 7 days.
      </p>
