  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (path.endsWith("/wallets/"))
        return { ok: true, json: async () => ({ wallets: [wallet] }) } as Response;
      if (path.endsWith("/captures/"))
        return {
          ok: true,
          json: async () => ({
            captures: [
              { id: "capture-1", householdId, amountMinor: 50000, status: "pending" },
            ],
          }),
        } as Response;
      if (path.endsWith("/reconciliations/"))
        return { ok: true, json: async () => ({ reconciliations: [] }) } as Response;
...
describe("Dashboard overview", () => {
  it("shows trusted balance distinctions and summary links", async () => {
    stubDashboard();
    render(<HouseholdOverview householdId={householdId} />);
    expect(
      await screen.findByRole("heading", { name: /your financial position/i }),
    ).toBeInTheDocument();
  ...
  it("keeps empty financial states useful", async () => {
    stubDashboard({ balances: [], currentBudgets: [], activeGoals: [], activity: [] });
    render(<HouseholdOverview householdId={householdId} />);
    expect(
      await screen.findByRole("heading", { name: /start your household overview/i }),
    ).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /add a wallet/i })).toHaveAttribute(
      "href",
      "/wallets",
    );
