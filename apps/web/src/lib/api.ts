...
export type WalletBalance = { walletId: string; amountMinor: number; reservedMinor?: number; availableMinor?: number; currency: string };
...
export type GoalSummary = { id: string; householdId: string; name: string; currency: string; targetAmountMinor: number; status: GoalStatus; archivedAt?: string; reservedMinor: number; remainingMinor: number };
type ApiErrorBody = { error?: string };
async function request<T>(path:string,init?:RequestInit):Promise<T>{const response=await fetch(path,{...init,headers:{'Content-Type':'application/json',...init?.headers}});if(!response.ok){let code=`http_${response.status}`;try{const body=(await response.json()) as ApiErrorBody;code=body.error||code}catch{}throw new Error(code)}return response.json() as Promise<T>}
...
export function createGoal(householdId:string,input:{name:string;currency:string;targetAmountMinor:number}){return request<GoalSummary>(`/api/households/${householdId}/goals/`,{method:'POST',body:JSON.stringify(input)})}
export async function listGoals(householdId:string){return(await request<{goals:GoalSummary[]}>(`/api/households/${householdId}/goals/`)).goals}
export function updateGoal(householdId:string,goalId:string,input:{name:string;targetAmountMinor:number}){return request<GoalSummary>(`/api/households/${householdId}/goals/${goalId}`,{method:'PATCH',body:JSON.stringify(input)})}
export function archiveGoal(householdId:string,goalId:string){return request<GoalSummary>(`/api/households/${householdId}/goals/${goalId}/archive`,{method:'POST'})}
export function reserveGoal(householdId:string,goalId:string,input:{walletId:string;amountMinor:number}){return request<GoalSummary>(`/api/households/${householdId}/goals/${goalId}/reserve`,{method:'POST',body:JSON.stringify(input)})}
export function releaseGoal(householdId:string,goalId:string,input:{walletId:string;amountMinor:number}){return request<GoalSummary>(`/api/households/${householdId}/goals/${goalId}/release`,{method:'POST',body:JSON.stringify(input)})}
