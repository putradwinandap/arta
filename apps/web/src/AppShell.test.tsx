...
describe("AppShell", () => {
  ...
  it("exposes the primary destinations and account control", () => {
    render(
      <AppShell email="family@example.com" offline={false} onLogout={vi.fn()}>
        <p>content</p>
      </AppShell>,
    );
    expect(screen.getAllByRole("link", { name: "Dashboard" })).toHaveLength(1);
    expect(screen.getAllByRole("link", { name: "Transactions & Inbox" })).toHaveLength(
      1,
    );
  ...
  it("keeps the active destination in sync with browser history", () => {
    ...
    expect(screen.getByRole("link", { name: "Transactions & Inbox" })).toHaveAttribute(
      "aria-current",
      "page",
    );
  });
