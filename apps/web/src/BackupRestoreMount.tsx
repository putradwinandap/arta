...
export function BackupRestoreMount() {
  ...
  return <main id="family" className="app-shell family-shell"><section className="panel" aria-labelledby="backup-restore-title"><h2 id="backup-restore-title">Backup & Restore</h2><p>Lindungi seluruh catatan keuangan household tanpa akses langsung ke database.</p><div className="actions"><button disabled={busy} onClick={download}>Download backup</button><label>File backup <input type="file" accept="application/json,.json" disabled={busy} onChange={select} /></label><button disabled={busy || !fileText} onClick={restore}>Restore backup</button></div><p role="status" aria-live="polite">{message}</p><p><strong>Perhatian:</strong> restore mengganti data household saat ini. Arta memvalidasi format dan versi sebelum perubahan dilakukan.</p></section></main>;
}
