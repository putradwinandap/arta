...
export function WalletSection(p: WalletSectionProps) {
  return (
                  ...
                  <label>
                    ...
                    <select
                      ...
                      onChange={(e: ChangeEvent<HTMLSelectElement>) =>
                        p.setEditType(e.target.value as WalletType)
                      }
                    >
                      {walletTypes.map((x) => (
        ...
        <form className="stack-form" onSubmit={p.handleCreateWallet}>
          <label>
            ...
            <input
              value={p.walletName}
              onChange={(e: ChangeEvent<HTMLInputElement>) =>
                p.setWalletName(e.target.value)
              }
              required
              maxLength={120}
            />
          ...
          <label>
            ...
            <select
              value={p.walletType}
              onChange={(e: ChangeEvent<HTMLSelectElement>) =>
                p.setWalletType(e.target.value as WalletType)
              }
            >
              {walletTypes.map((x) => (
                <option key={x.value} value={x.value}>
          ...
          <label>
            ...
            <input
              value={p.currency}
              onChange={(e: ChangeEvent<HTMLInputElement>) =>
                p.setCurrency(e.target.value.toUpperCase())
              }
              maxLength={3}
              required
            />
