...
export function useFinancialWorkspace() {
  const [budgets, setBudgets] = useState<BudgetSummary[]>([]),
    [transactionBudgetId, setTransactionBudgetId] = useState("");
  const [memberships, setMemberships] = useState<HouseholdMembership[]>([]),
    [household, setHousehold] = useState<Household | null>(null);
  const [wallets, setWallets] = useState<Wallet[]>([]),
    [overview, setOverview] = useState<FinanceOverview>({
      balances: [],
  ...
  const [online, setOnline] = useState(() => navigator.onLine),
    [showHouseholdManagement, setShowHouseholdManagement] = useState(false);
  ...
  const archivedWallets = useMemo(
    () => wallets.filter((w) => w.status === "archived"),
    [wallets],
  );
  const balanceByWallet = useMemo(
    () => new Map(overview.balances.map((b) => [b.walletId, b])),
    [overview],
  );
  async function refresh(id: string) {
    const [w, o, b] = await Promise.all([
      listWallets(id),
  ...
  async function activateHousehold(id: string) {
    ...
    setError("");
    try {
      const ms = await listMyHouseholds();
      setMemberships(ms);
      const selected = ms.find((m) => m.id === id);
      ...
      localStorage.setItem("arta.activeHouseholdName", h.name);
      setHousehold(h);
      setWallets(w);
      setOverview(o);
    ...
    listMyHouseholds()
      .then(async (ms) => {
        ...
        const saved = localStorage.getItem(HOUSEHOLD_STORAGE_KEY),
          selected = ms.find((m) => m.id === saved) ?? ms[0];
        const [h, w, o] = await Promise.all([
      ...
      .catch((e) => {
        if (!(e instanceof Error && /^(http_4|unauthenticated)/.test(e.message))) {
          try {
            const c = JSON.parse(
            ...
            if (
              c?.userId &&
              c.userId === localStorage.getItem("arta.activeUserId") &&
              c.household?.id &&
              c.wallets &&
              c.overview
            ) {
              setMemberships(c.memberships || []);
              setHousehold(c.household);
              setWallets(c.wallets);
              setOverview(c.overview);
              setError(
                `Offline mode. Showing data saved ${new Date(c.savedAt).toLocaleString()}.`,
              );
              return;
            }
          } catch {}
  ...
  }, [activeWallets, transactionWalletId, transferSourceId, transferDestinationId]);
  useEffect(() => {
  ...
  async function action(fn: () => Promise<void>, after?: () => void) {
    setSaving(true);
    setError("");
    try {
      await fn();
  ...
  const beginEdit = (w: Wallet) => {
    setEditingWalletId(w.id);
    setEditName(w.name);
    setEditType(w.type);
  };
  const handleUpdateWallet = (e: FormEvent, id: string) => {
    e.preventDefault();
    if (household)
      void action(
        async () => {
          await updateWallet(household.id, id, { name: editName, type: editType });
          await refresh(household.id);
        },
  ...
  const handleArchive = (id: string) => {
    if (household)
      void action(
        async () => {
          await archiveWallet(household.id, id);
          await refresh(household.id);
        },
        () => setEditingWalletId(null),
      );
