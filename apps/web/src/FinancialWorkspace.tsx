...
export function FinancialWorkspace({
  ...
  async function refresh(householdId: string) {
    const [nextWallets, nextOverview, nextBudgets] = await Promise.all([
  ...
  async function activateHousehold(householdId: string) {
    setSwitching(true);
    setError("");
    try {
      const nextMemberships = await listMyHouseholds();
      setMemberships(nextMemberships);
      const selected = nextMemberships.find((item) => item.id === householdId);
  ...
  useEffect(() => {
    listMyHouseholds()
      .then(async (nextMemberships) => {
        ...
        const selected =
          nextMemberships.find((item) => item.id === savedId) ??
          nextMemberships[0];
        const [storedHousehold, storedWallets, storedOverview] =
          await Promise.all([
            getHousehold(selected.id),
            listWallets(selected.id),
            getFinanceOverview(selected.id),
        ...
        localStorage.setItem(HOUSEHOLD_STORAGE_KEY, selected.id);
        localStorage.setItem("arta.activeHouseholdName", storedHousehold.name);
      ...
      .catch((err) => {
        const serverRejected =
          err instanceof Error && /^(http_4|unauthenticated)/.test(err.message);
        if (!serverRejected) {
          try {
            ...
            if (
              cached?.userId &&
              cached.userId === localStorage.getItem("arta.activeUserId") &&
              cached.household?.id &&
              cached.wallets &&
              cached.overview
            ) {
              localStorage.setItem(
                "arta.activeHouseholdName",
                cached.household.name,
              );
              ...
              setError(
                `Offline mode. Showing data saved ${new Date(cached.savedAt).toLocaleString()}.`,
              );
              return;
            }
          } catch {}
  ...
  useEffect(() => {
    ...
    if (!transferDestinationId && activeWallets.length > 1)
      setTransferDestinationId(activeWallets[1].id);
  }, [
    activeWallets,
    transactionWalletId,
  ...
  async function handleCreateWallet(event: FormEvent) {
    ...
    setError("");
    try {
      await createWallet(household.id, {
        name: walletName,
        type: walletType,
        currency,
      });
  ...
  async function handleUpdateWallet(event: FormEvent, walletId: string) {
    ...
    setError("");
    try {
      await updateWallet(household.id, walletId, {
        name: editName,
        type: editType,
      });
      await refresh(household.id);
  ...
  async function handleTransaction(event: FormEvent) {
    ...
    try {
      await createTransaction(household.id, {
        ...
        amountMinor: Number(transactionAmount),
  ...
  async function handleTransfer(event: FormEvent) {
    ...
    try {
      await createTransfer(household.id, {
        sourceWalletId: transferSourceId,
        destinationWalletId: transferDestinationId,
  ...
  const pageTitle =
    view === "wallets"
      ...
      : view === "transactions"
        ...
        : household?.name;
  useEffect(() => {
    if (pageTitle) document.title = `${pageTitle} · Arta`;
  }, [pageTitle]);
  ...
  if (!household)
    return (
      <main className="onboarding-shell">
        <HouseholdEntry onActivated={activateHousehold} />
        {error && (
  ...
  if (view === "dashboard")
    return <HouseholdOverview householdId={household.id} />;
    ...
    <main className="app-shell">
      <header className="topbar">
        <div>
          ...
          <p className="muted">
            Capture quickly, review later, and keep confirmed money trustworthy.
          </p>
        </div>
        <div className="household-controls">
          {memberships.length > 1 && (
            <label className="household-switcher">
              ...
              <select
                ...
                value={household.id}
                disabled={switching}
                onChange={(event) => activateHousehold(event.target.value)}
              >
          ...
          <button
            type="button"
            className="secondary"
            onClick={() => setShowHouseholdManagement((value) => !value)}
          >
            {showHouseholdManagement
              ? "Close household management"
              : "Manage households"}
          </button>
      ...
      {showHouseholdManagement && (
        <HouseholdEntry embedded onActivated={activateHousehold} />
      )}
      <QuickCaptureInbox
        householdId={household.id}
        wallets={activeWallets}
        onConfirmed={() => refresh(household.id)}
      />
      <section className="finance-summary">
        ...
        <article className="summary-card">
          ...
          <strong>
            {formatMoney(
              overview.totals.incomeMinor - overview.totals.expenseMinor,
            )}
          </strong>
        ...
        <section className="panel">
          ...
          {wallets.length === 0 ? (
            <div className="empty-state">
              ...
              <p>
                Add at least one wallet before confirming financial activity.
              </p>
            </div>
          ) : (
              ...
              {activeWallets.map((wallet) => (
                <article className="wallet-card" key={wallet.id}>
                  {editingWalletId === wallet.id ? (
                    <form
                      className="edit-form"
                      onSubmit={(event) => handleUpdateWallet(event, wallet.id)}
                    >
                      <label>
                        Wallet name
                        <input
                      ...
                      <label>
                        ...
                        <select
                          value={editType}
                          onChange={(event) =>
                            setEditType(event.target.value as WalletType)
                          }
                        >
                    ...
                    <>
                      <div>
                        ...
                        <p className="wallet-balance">
                          {formatMoney(
                            balanceByWallet.get(wallet.id)?.amountMinor ?? 0,
                            wallet.currency,
                          )}
                        </p>
                      </div>
                ...
                <div className="archived-section">
                  ...
                  {archivedWallets.map((wallet) => (
                    <article className="wallet-card archived" key={wallet.id}>
                      <div>
                        <span className="wallet-type">Archived</span>
                        <h3>{wallet.name}</h3>
                        <p className="muted">
                          {formatMoney(
                            balanceByWallet.get(wallet.id)?.amountMinor ?? 0,
      ...
      {activeWallets.length > 0 && (
        <section className="finance-grid">
          <section className="panel transaction-panel">
            ...
            <form className="stack-form" onSubmit={handleTransaction}>
              {transactionKind === "expense" && (
                <label>
                  Budget
                  <select
                    aria-label="Transaction budget"
                    value={transactionBudgetId}
                    onChange={(event) => setTransactionBudgetId(event.target.value)}
                  >
                    ...
                    {budgets
                      .filter(
                        (budget) =>
                          budget.currency ===
                          activeWallets.find((wallet) => wallet.id === transactionWalletId)
                            ?.currency,
                      )
                      .map((budget) => (
                        <option value={budget.id} key={budget.id}>
                          {budget.periodStart.slice(0, 10)} → {budget.periodEnd.slice(0, 10)} · {budget.currency}
                        </option>
                      ))}
                  </select>
                </label>
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setTransactionWalletId(event.target.value)
                  }
                >
                  {activeWallets.map((wallet) => (
              ...
              <button disabled={saving}>Record {transactionKind}</button>
            </form>
          </section>
          <section className="panel transfer-panel">
            ...
            {activeWallets.length < 2 ? (
              ...
              <form className="stack-form" onSubmit={handleTransfer}>
                ...
                <button disabled={saving}>Transfer money</button>
              </form>
            )}
          </section>
        </section>
