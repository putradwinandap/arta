...
export function App() {
  ...
  async function handleUpdateWallet(event: FormEvent, walletId: string) {
    event.preventDefault(); if (!household) return; setSaving(true); setError('');
    try { await updateWallet(household.id, walletId, { name: editName, type: editType }); await refresh(household.id); setEditingWalletId(null); }
    catch (err) { setError(errorMessage(err)); } finally { setSaving(false); }
  }
  ...
  async function handleTransaction(event: FormEvent) {
    event.preventDefault(); if (!household) return; setSaving(true); setError('');
    try { await createTransaction(household.id, { walletId: transactionWalletId, kind: transactionKind, amountMinor: Number(transactionAmount), note: transactionNote }); await refresh(household.id); setTransactionAmount(''); setTransactionNote(''); }
    catch (err) { setError(errorMessage(err)); } finally { setSaving(false); }
  }

  async function handleTransfer(event: FormEvent) {
    event.preventDefault(); if (!household) return; setSaving(true); setError('');
    try { await createTransfer(household.id, { sourceWalletId: transferSourceId, destinationWalletId: transferDestinationId, amountMinor: Number(transferAmount), note: transferNote }); await refresh(household.id); setTransferAmount(''); setTransferNote(''); }
    catch (err) { setError(errorMessage(err)); } finally { setSaving(false); }
  }
  ...
  if (!household) {
    return <main className="onboarding-shell"><section className="onboarding-card">
      ...
      <p className="muted">Create a household first. Your signed-in account becomes its owner and all household access is verified by the server.</p>
  ...
  return <main className="app-shell">
    ...
    <section className="finance-summary"><article className="summary-card"><span>Income</span><strong>{formatMoney(overview.totals.incomeMinor)}</strong></article><article className="summary-card"><span>Expense</span><strong>{formatMoney(overview.totals.expenseMinor)}</strong></article><article className="summary-card"><span>Net</span><strong>{formatMoney(overview.totals.incomeMinor - overview.totals.expenseMinor)}</strong><small>Transfers and pending captures excluded</small></article></section>
    <section className="layout-grid"><section className="panel"><div className="section-heading"><div><p className="eyebrow">Wallets</p><h2>Your money locations</h2></div></div>{wallets.length === 0 ? <div className="empty-state"><h3>No wallets yet</h3><p>Add at least one wallet before confirming financial activity.</p></div> : <div className="wallet-list">{activeWallets.map((wallet) => <article className="wallet-card" key={wallet.id}>{editingWalletId === wallet.id ? <form className="edit-form" onSubmit={(event) => handleUpdateWallet(event, wallet.id)}><label>Wallet name<input value={editName} onChange={(event) => setEditName(event.target.value)} required maxLength={120} /></label><label>Type<select value={editType} onChange={(event) => setEditType(event.target.value as WalletType)}>{walletTypes.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><div className="button-row"><button type="submit" disabled={saving}>Save</button><button type="button" className="secondary" onClick={() => setEditingWalletId(null)}>Cancel</button></div></form> : <><div><span className="wallet-type">{walletTypes.find((item) => item.value === wallet.type)?.label}</span><h3>{wallet.name}</h3><p className="wallet-balance">{formatMoney(balanceByWallet.get(wallet.id)?.amountMinor ?? 0, wallet.currency)}</p></div><div className="wallet-actions"><button className="secondary" type="button" onClick={() => beginEdit(wallet)}>Edit</button><button className="danger" type="button" onClick={() => handleArchive(wallet.id)} disabled={saving}>Archive</button></div></>}</article>)}{archivedWallets.length > 0 && <div className="archived-section"><h3>Archived</h3>{archivedWallets.map((wallet) => <article className="wallet-card archived" key={wallet.id}><div><span className="wallet-type">Archived</span><h3>{wallet.name}</h3><p className="muted">{formatMoney(balanceByWallet.get(wallet.id)?.amountMinor ?? 0, wallet.currency)} · read only</p></div></article>)}</div>}</div>}</section><aside className="panel create-panel"><p className="eyebrow">Add wallet</p><h2>Create a money location</h2><form className="stack-form" onSubmit={handleCreateWallet}><label>Wallet name<input value={walletName} onChange={(event) => setWalletName(event.target.value)} placeholder="BCA Utama" maxLength={120} required /></label><label>Type<select value={walletType} onChange={(event) => setWalletType(event.target.value as WalletType)}>{walletTypes.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><label>Currency<input value={currency} onChange={(event) => setCurrency(event.target.value.toUpperCase())} maxLength={3} pattern="[A-Za-z]{3}" required /></label><button type="submit" disabled={saving}>{saving ? 'Saving…' : 'Add wallet'}</button></form></aside></section>
    {activeWallets.length > 0 && <section className="finance-grid"><section className="panel transaction-panel"><p className="eyebrow">Record confirmed money</p><h2>Income or expense</h2><form className="stack-form" onSubmit={handleTransaction}><label>Type<select aria-label="Transaction type" value={transactionKind} onChange={(event) => setTransactionKind(event.target.value as TransactionKind)}><option value="expense">Expense</option><option value="income">Income</option></select></label><label>Wallet<select aria-label="Transaction wallet" value={transactionWalletId} onChange={(event) => setTransactionWalletId(event.target.value)}>{activeWallets.map((wallet) => <option value={wallet.id} key={wallet.id}>{wallet.name}</option>)}</select></label><label>Amount<input aria-label="Transaction amount" type="number" min="1" step="1" value={transactionAmount} onChange={(event) => setTransactionAmount(event.target.value)} required /></label><label>Note<input aria-label="Transaction note" value={transactionNote} onChange={(event) => setTransactionNote(event.target.value)} placeholder="Groceries" /></label><button disabled={saving}>Record {transactionKind}</button></form></section><section className="panel transfer-panel"><p className="eyebrow">Move money</p><h2>Wallet transfer</h2>{activeWallets.length < 2 ? <div className="empty-state"><p>Add another active wallet to transfer money.</p></div> : <form className="stack-form" onSubmit={handleTransfer}><label>From<select aria-label="Transfer source" value={transferSourceId} onChange={(event) => setTransferSourceId(event.target.value)}>{activeWallets.map((wallet) => <option value={wallet.id} key={wallet.id}>{wallet.name}</option>)}</select></label><label>To<select aria-label="Transfer destination" value={transferDestinationId} onChange={(event) => setTransferDestinationId(event.target.value)}>{activeWallets.map((wallet) => <option value={wallet.id} key={wallet.id}>{wallet.name}</option>)}</select></label><label>Amount<input aria-label="Transfer amount" type="number" min="1" step="1" value={transferAmount} onChange={(event) => setTransferAmount(event.target.value)} required /></label><label>Note<input aria-label="Transfer note" value={transferNote} onChange={(event) => setTransferNote(event.target.value)} placeholder="Move to savings" /></label><button disabled={saving}>Transfer money</button></form>}</section></section>}
    <section className="panel activity-panel"><p className="eyebrow">Confirmed history</p><h2>Recent activity</h2>{overview.activity.length === 0 ? <div className="empty-state"><p>No confirmed financial activity yet.</p></div> : <div className="activity-list">{overview.activity.map((item) => { const wallet = item.walletId ? walletById.get(item.walletId) : undefined; const source = item.sourceWalletId ? walletById.get(item.sourceWalletId) : undefined; const destination = item.destinationWalletId ? walletById.get(item.destinationWalletId) : undefined; const title = item.type === 'transfer' ? `${source?.name ?? 'Wallet'} → ${destination?.name ?? 'Wallet'}` : `${item.type === 'income' ? 'Income' : 'Expense'} · ${wallet?.name ?? 'Wallet'}`; return <article className="activity-row" key={item.id}><div><strong>{title}</strong><p className="muted">{item.note || new Date(item.occurredAt).toLocaleString()}</p></div><strong>{item.type === 'expense' ? '-' : ''}{formatMoney(item.amountMinor, item.currency)}</strong></article>; })}</div>}</section>
  </main>;
}
