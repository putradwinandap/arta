...
type Props = {
  embedded?: boolean;
  onActivated: (householdId: string) => Promise<void>;
};
...
export function HouseholdEntry({ embedded = false, onActivated }: Props) {
  ...
  useEffect(() => {
    if (!embedded) return;
    const id = localStorage.getItem("arta.householdId") || "";
    void listMyHouseholds()
      .then((items) => {
        if (items.some((item) => item.id === id && item.role === "owner"))
          setOwnerHouseholdId(id);
      })
      .catch(() => undefined);
  }, [embedded]);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const householdId =
        mode === "create"
          ? (await createHousehold(name)).id
          : (await joinHousehold(token)).householdId;
      setName("");
      setToken("");
      await onActivated(householdId);
    } catch (err) {
      setError(
        err instanceof Error
          ? err.message.replaceAll("_", " ")
          : "Could not update household membership.",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <section
      className={embedded ? "panel household-entry" : "onboarding-card household-entry"}
    >
      <p className="eyebrow">{embedded ? "Household management" : "Household setup"}</p>
      <h2>
        {embedded ? "Create or join another household" : "Choose how to get started"}
      </h2>
