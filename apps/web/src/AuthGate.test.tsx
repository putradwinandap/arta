...
describe('AuthGate offline recovery', () => {
  it('keeps the last authenticated user when auth validation cannot reach the server', async () => {
    localStorage.setItem('arta.authUser', JSON.stringify({ id: 'user-1', email: 'family@example.com' }));
  ...
  it('does not restore the cached user after a server 401', async () => {
    localStorage.setItem('arta.authUser', JSON.stringify({ id: 'user-1', email: 'family@example.com' }));
    vi.stubGlobal('fetch', vi.fn(async () => ({ ok: false, status: 401, json: async () => ({ error: 'unauthenticated' }) }) as Response));
    render(<AuthGate><p>dashboard</p></AuthGate>);
  ...
  it('clears auth and household snapshots on logout', async () => {
    localStorage.setItem('arta.appSnapshot', JSON.stringify({ household: { id: 'household-1' } }));
    vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL) => {
      if (String(input) === '/api/auth/me') return { ok: true, status: 200, json: async () => ({ id: 'user-1', email: 'family@example.com' }) } as Response;
      if (String(input) === '/api/auth/logout') return { ok: true, status: 204 } as Response;
