...
- Issue #55 / PR #63: typed financial component boundaries, merged and closed.
...
- Offline verification: web typecheck, frontend tests, production PWA build, and Playwright E2E against `http://127.0.0.1:8080` pass. The E2E flow covers login, household setup, financial activity, API interruption, reload, cached activity visibility, offline Quick Capture, reconnect sync, and logout.
...
- Global card visual standard: financial cards share the `surface-card` visual baseline (border, radius, background, and subtle elevation); interactive treatment remains explicit and is applied only to navigable cards.
