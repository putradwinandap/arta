...
func TestHouseholdOverviewDerivesTrustedFinancialState(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = pool.Exec(context.Background(), `DELETE FROM wallet_reconciliations WHERE household_id=$1`, house.ID)
	...
	if _, err = service.ReserveGoalFunds(ctx, house.ID, goalSummary.ID, savings.ID, 150_000); err != nil {
		t.Fatal(err)
	}
	...
	if len(overview.Balances) != 2 {
		t.Fatalf("expected two balances, got %d", len(overview.Balances))
	}
	var physical, reserved, available int64
	for _, b := range overview.Balances {
		physical += b.AmountMinor
		reserved += b.ReservedMinor
		available += b.AvailableMinor
	}
	if physical != 900_000 || reserved != 150_000 || available != 750_000 {
		t.Fatalf("unexpected balance totals: physical=%d reserved=%d available=%d", physical, reserved, available)
	}
	if len(overview.CurrentBudgets) != 1 || overview.CurrentBudgets[0].ID != budgetSummary.ID || overview.CurrentBudgets[0].SpentMinor != 100_000 {
		t.Fatalf("unexpected current budget: %+v", overview.CurrentBudgets)
	}
	if len(overview.ActiveGoals) != 1 || overview.ActiveGoals[0].ReservedMinor != 150_000 {
		t.Fatalf("unexpected active goals: %+v", overview.ActiveGoals)
	}
	if len(overview.RecentActivity) != 3 {
		t.Fatalf("expected three recent activities, got %d", len(overview.RecentActivity))
	}
	seen := map[string]bool{}
	for _, item := range overview.RecentActivity {
		seen[item.Type] = true
	}
	if !seen["income"] || !seen["expense"] || !seen["transfer"] {
		t.Fatalf("activity semantics missing: %+v", seen)
	}
}
