...
func (s *Service) walletPhysicalBalance(ctx context.Context, q interface {
	QueryRow(context.Context, string, ...any) pgx.Row
}, householdID, walletID uuid.UUID) (int64, error) {
	var balance int64
...
func (s *Service) ListReconciliations(ctx context.Context, householdID uuid.UUID) ([]Reconciliation, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.HouseholdID, &r.WalletID, &r.Currency, &r.ExpectedAmountMinor, &r.ObservedAmountMinor, &r.DiscrepancyMinor, &r.Status, &r.AdjustmentID, &r.ObservedAt, &r.ResolvedAt); err != nil {
			return nil, err
		}
		out = append(out, r)
	}
...
func (s *Service) ResolveReconciliationWithAdjustment(ctx context.Context, householdID, reconciliationID uuid.UUID, reason string) (Reconciliation, error) {
	...
	if reason == "" {
		return Reconciliation{}, ErrInvalidAdjustmentReason
	}
	...
	if r.Status != "unresolved" {
		return Reconciliation{}, ErrAlreadyReconciled
	}
	current, err := s.walletPhysicalBalance(ctx, tx, householdID, r.WalletID)
	if err != nil {
		return Reconciliation{}, err
	}
	...
	if remaining == 0 {
		...
		r.Status = "reconciled"
		r.ResolvedAt = &now
		_, err = tx.Exec(ctx, `UPDATE wallet_reconciliations SET status='reconciled',resolved_at=$1 WHERE id=$2`, now, r.ID)
		if err != nil {
			return Reconciliation{}, err
		}
	...
	now := s.now().UTC()
	if _, err = tx.Exec(ctx, `INSERT INTO balance_adjustments (id,household_id,wallet_id,amount_minor,currency,reason,created_at) VALUES ($1,$2,$3,$4,$5,$6,$7)`, adjustmentID, householdID, r.WalletID, remaining, r.Currency, reason, now); err != nil {
		return Reconciliation{}, err
	}
	if _, err = tx.Exec(ctx, `UPDATE wallet_reconciliations SET status='reconciled',adjustment_id=$1,resolved_at=$2 WHERE id=$3`, adjustmentID, now, r.ID); err != nil {
		return Reconciliation{}, err
	}
	if err = tx.Commit(ctx); err != nil {
		return Reconciliation{}, err
	...
	r.Status = "reconciled"
	r.AdjustmentID = &adjustmentID
	r.ResolvedAt = &now
	return r, nil
}

func IsReconciliationInputError(err error) bool {
	return errors.Is(err, ErrInvalidObservedBalance) || errors.Is(err, ErrInvalidAdjustmentReason) || errors.Is(err, ErrAlreadyReconciled)
}
