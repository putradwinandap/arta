...
func (s *Service) ListGoals(ctx context.Context, householdID uuid.UUID) ([]goal.Summary, error) {
	...
	for rows.Next() {
		var value goal.Goal
		var reserved int64
		if err := rows.Scan(&value.ID, &value.HouseholdID, &value.Name, &value.Currency, &value.TargetAmountMinor, &value.Status, &value.ArchivedAt, &reserved); err != nil {
			return nil, err
		}
...
func (s *Service) UpdateGoal(ctx context.Context, householdID, goalID uuid.UUID, name string, target int64) (goal.Summary, error) {
	current, err := s.GetGoal(ctx, householdID, goalID)
	if err != nil {
		return goal.Summary{}, err
	}
	if current.Status == goal.StatusArchived {
		return goal.Summary{}, goal.ErrArchived
	}
	name = strings.TrimSpace(name)
	if name == "" {
		return goal.Summary{}, goal.ErrInvalidName
	}
	if target <= 0 {
		return goal.Summary{}, goal.ErrInvalidTarget
	}
...
func (s *Service) ReserveGoalFunds(ctx context.Context, householdID, goalID, walletID uuid.UUID, amount int64) (goal.Summary, error) {
	if amount <= 0 {
		return goal.Summary{}, goal.ErrInvalidAmount
	}
	...
	var currency string
	if err := tx.QueryRow(ctx, `SELECT status,currency FROM financial_goals WHERE household_id=$1 AND id=$2 FOR UPDATE`, householdID, goalID).Scan(&status, &currency); err != nil {
		return goal.Summary{}, err
	}
	if status == goal.StatusArchived {
		return goal.Summary{}, goal.ErrArchived
	}
	var walletCurrency string
	var walletStatus wallet.Status
	if err := tx.QueryRow(ctx, `SELECT currency,status FROM wallets WHERE household_id=$1 AND id=$2 FOR UPDATE`, householdID, walletID).Scan(&walletCurrency, &walletStatus); err != nil {
		return goal.Summary{}, err
	}
	...
	if walletCurrency != currency {
		return goal.Summary{}, goal.ErrInvalidCurrency
	}
	var balance, reserved int64
	if err := tx.QueryRow(ctx, `SELECT COALESCE((SELECT SUM(CASE WHEN kind='income' THEN amount_minor ELSE -amount_minor END) FROM transactions WHERE household_id=$1 AND wallet_id=$2),0)+COALESCE((SELECT SUM(CASE WHEN destination_wallet_id=$2 THEN amount_minor ELSE -amount_minor END) FROM transfers WHERE household_id=$1 AND (source_wallet_id=$2 OR destination_wallet_id=$2)),0)`, householdID, walletID).Scan(&balance); err != nil {
		return goal.Summary{}, err
	}
	if err := tx.QueryRow(ctx, `SELECT COALESCE(SUM(CASE WHEN kind='reserve' THEN amount_minor ELSE -amount_minor END),0) FROM goal_reservation_events WHERE household_id=$1 AND wallet_id=$2`, householdID, walletID).Scan(&reserved); err != nil {
		return goal.Summary{}, err
	}
	if balance-reserved < amount {
		return goal.Summary{}, goal.ErrInsufficientAvailable
	}
	_, err = tx.Exec(ctx, `INSERT INTO goal_reservation_events (id,goal_id,household_id,wallet_id,kind,amount_minor) VALUES ($1,$2,$3,$4,'reserve',$5)`, uuid.New(), goalID, householdID, walletID, amount)
	if err != nil {
		return goal.Summary{}, err
	}
...
func (s *Service) ReleaseGoalFunds(ctx context.Context, householdID, goalID, walletID uuid.UUID, amount int64) (goal.Summary, error) {
	...
	defer tx.Rollback(ctx)
	var currency string
	if err := tx.QueryRow(ctx, `SELECT currency FROM financial_goals WHERE household_id=$1 AND id=$2 FOR UPDATE`, householdID, goalID).Scan(&currency); err != nil {
		return goal.Summary{}, err
	}
	var walletCurrency string
	if err := tx.QueryRow(ctx, `SELECT currency FROM wallets WHERE household_id=$1 AND id=$2 FOR UPDATE`, householdID, walletID).Scan(&walletCurrency); err != nil {
		return goal.Summary{}, err
	}
	if walletCurrency != currency {
		return goal.Summary{}, goal.ErrInvalidCurrency
	...
	var reserved int64
	if err := tx.QueryRow(ctx, `SELECT COALESCE(SUM(CASE WHEN kind='reserve' THEN amount_minor ELSE -amount_minor END),0) FROM goal_reservation_events WHERE household_id=$1 AND goal_id=$2 AND wallet_id=$3`, householdID, goalID, walletID).Scan(&reserved); err != nil {
		return goal.Summary{}, err
	}
	if reserved < amount {
		return goal.Summary{}, goal.ErrInsufficientReserved
	}
	_, err = tx.Exec(ctx, `INSERT INTO goal_reservation_events (id,goal_id,household_id,wallet_id,kind,amount_minor) VALUES ($1,$2,$3,$4,'release',$5)`, uuid.New(), goalID, householdID, walletID, amount)
	if err != nil {
		return goal.Summary{}, err
	}
	if err := tx.Commit(ctx); err != nil {
...
func (s *Service) WalletAvailableBalances(ctx context.Context, householdID uuid.UUID) ([]WalletBalance, error) {
	balances, err := s.WalletBalances(ctx, householdID)
	if err != nil {
		return nil, err
	}
	for i := range balances {
		var reserved int64
		if err := s.pool.QueryRow(ctx, `SELECT COALESCE(SUM(CASE WHEN kind='reserve' THEN amount_minor ELSE -amount_minor END),0) FROM goal_reservation_events WHERE household_id=$1 AND wallet_id=$2`, householdID, balances[i].WalletID).Scan(&reserved); err != nil {
			return nil, err
		}
		balances[i].ReservedMinor = reserved
		balances[i].AvailableMinor = balances[i].AmountMinor - reserved
	}
	return balances, nil
}
