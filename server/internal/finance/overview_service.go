...
// GetHouseholdOverview derives one landing-page view from the existing trusted
...
func (s *Service) GetHouseholdOverview(ctx context.Context, householdID uuid.UUID) (HouseholdOverview, error) {
	...
	if err != nil { return HouseholdOverview{}, err }
	...
	for _, item := range goals {
		if item.Status == goal.StatusActive { activeGoals = append(activeGoals, item) }
	}
	if len(activity) > 10 { activity = activity[:10] }

	return HouseholdOverview{Balances: balances, Totals: totals, CurrentBudgets: currentBudgets, ActiveGoals: activeGoals, RecentActivity: activity}, nil
}
