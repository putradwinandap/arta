...
func TestBudgetCountsOnlyEligibleConfirmedExpenses(t *testing.T) {
	...
	captureID := uuid.New()
	_, _ = service.CreateCapture(ctx, captureID, house.ID, 80_000, "pending", inside)

	summary, err := service.GetBudget(ctx, house.ID, created.ID)
	if err != nil {
		t.Fatal(err)
	}
	if summary.SpentMinor != 125_000 || summary.RemainingMinor != 375_000 {
		t.Fatalf("budget counted ineligible money: %+v", summary)
	}
