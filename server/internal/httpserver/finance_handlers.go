...
func (h financeHandlers) createHousehold(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { writeError(w, http.StatusBadRequest, "invalid_owner_subject_id"); return }
...
func (h financeHandlers) getWallet(w http.ResponseWriter, r *http.Request) {
	householdID, ok := pathUUID(w, r, "householdID"); if !ok { return }
	walletID, ok := pathUUID(w, r, "walletID"); if !ok { return }
...
func (h financeHandlers) createTransfer(w http.ResponseWriter, r *http.Request) {
	...
	if err := decodeJSON(r, &input); err != nil { writeError(w, http.StatusBadRequest, "invalid_json"); return }
	sourceID, err := uuid.Parse(input.SourceWalletID); if err != nil { writeError(w, http.StatusBadRequest, "invalid_source_wallet_id"); return }
	destinationID, err := uuid.Parse(input.DestinationWalletID); if err != nil { writeError(w, http.StatusBadRequest, "invalid_destination_wallet_id"); return }
	occurredAt := time.Now(); if input.OccurredAt != nil { occurredAt = *input.OccurredAt }
...
func (h financeHandlers) getFinanceOverview(w http.ResponseWriter, r *http.Request) {
	householdID, ok := pathUUID(w, r, "householdID"); if !ok { return }
	balances, err := h.service.WalletBalances(r.Context(), householdID); if err != nil { handleFinanceError(w, err); return }
	totals, err := h.service.HouseholdTotals(r.Context(), householdID); if err != nil { handleFinanceError(w, err); return }
	activity, err := h.service.ListActivity(r.Context(), householdID); if err != nil { handleFinanceError(w, err); return }
	writeJSON(w, http.StatusOK, map[string]any{"balances": balances, "totals": totals, "activity": activity})
}

func pathUUID(w http.ResponseWriter, r *http.Request, key string) (uuid.UUID, bool) {
	id, err := uuid.Parse(chi.URLParam(r, key))
	if err != nil { writeError(w, http.StatusBadRequest, "invalid_"+key); return uuid.Nil, false }
	return id, true
}

func decodeJSON(r *http.Request, target any) error {
	decoder := json.NewDecoder(r.Body); decoder.DisallowUnknownFields(); return decoder.Decode(target)
}
...
func handleFinanceError(w http.ResponseWriter, err error) {
	...
	if errors.Is(err, household.ErrInvalidHouseholdName) || errors.Is(err, household.ErrInvalidSubjectID) || errors.Is(err, wallet.ErrInvalidName) || errors.Is(err, wallet.ErrInvalidType) || errors.Is(err, wallet.ErrInvalidCurrency) || errors.Is(err, wallet.ErrInvalidHousehold) || errors.Is(err, wallet.ErrWalletArchived) || finance.IsLedgerInputError(err) {
