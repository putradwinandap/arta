...
func New(householdID uuid.UUID, name, currency string, target int64) (Goal, error) {
	if householdID == uuid.Nil { return Goal{}, ErrInvalidHousehold }
	name = strings.TrimSpace(name)
	if name == "" { return Goal{}, ErrInvalidName }
	currency = strings.ToUpper(strings.TrimSpace(currency))
	if len(currency) != 3 { return Goal{}, ErrInvalidCurrency }
	for _, r := range currency { if r < 'A' || r > 'Z' { return Goal{}, ErrInvalidCurrency } }
	if target <= 0 { return Goal{}, ErrInvalidTarget }
