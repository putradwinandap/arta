...
func TestNewAndSummary(t *testing.T) {
	...
	if value.Currency != "IDR" { t.Fatalf("currency = %q", value.Currency) }
	if value.PeriodStart.Hour() != 0 || value.PeriodEnd.Hour() != 0 { t.Fatal("periods must be normalized to dates") }

	summary := Summarize(value, 1_250_000)
	if summary.RemainingMinor != -250_000 { t.Fatalf("remaining = %d", summary.RemainingMinor) }
}

func TestNewRejectsInvalidBoundaries(t *testing.T) {
	householdID := uuid.New()
	cases := []struct { name string; currency string; start, end time.Time; amount int64 }{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if _, err := New(householdID, tc.currency, tc.start, tc.end, tc.amount); err == nil { t.Fatal("expected error") }
		})
	}
}
